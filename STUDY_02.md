...
| 기술 용어 | 레스토랑 비유 | 역할 |
|---|---|---|
| HTML | 메뉴판 📋 | 어떤 내용을 보여줄지 구조를 잡아요 |
| CSS | 인테리어 🎨 | 색상, 폰트, 레이아웃으로 예쁘게 꾸며요 |
| JavaScript | 서빙 직원 🧑‍🍳 | 버튼 클릭 등 사용자 행동에 반응해요 |
| 백엔드 서버 | 주방 🍳 | 요청을 받아서 실제 데이터를 처리해요 |
| 데이터베이스 | 냉장고 🧊 | 모든 데이터를 저장하고 꺼내줘요 |
| Node.js | 가스레인지 🔥 | 백엔드 코드가 실제로 돌아가는 엔진 |

### 왜 이번엔 배포 방식이 달라질까요?

- **1회차:** HTML 파일만 있으니 Netlify에 폴더를 그냥 올리면 끝!
- **2회차~:** 로그인·데이터 저장 기능 추가 → 백엔드 서버가 필요해져요
- 백엔드가 생기면 파일을 **빌드(build)** 하는 과정이 필요하고, 배포 방식도 달라져요
...
- 외부 API 연동해보기 (네이버 지도, OPEN API 등)
